...
function wds_acme_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	return delete_transient( 'wds_acme_categories' );
}
...
function wds_acme_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'touchstonesfse' ) . '...</a>';
}
...
function wds_acme_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'touchstonesfse' ) );
}
 ...
 */
function wds_acme_get_the_content( $content ) {
	return ! empty( $content ) ? $content : false;
}

add_filter( 'the_content', 'wds_acme_get_the_content', 20 );
 ...
 */
function wds_acme_custom_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'wds_acme_custom_mime_types' );
 ...
 */
function wds_acme_display_customizer_header_scripts() {
	// Check for header scripts.
	$scripts = get_theme_mod( 'wds_acme_header_scripts' );
	...
	echo wds_acme_get_the_content( $scripts );
}

add_action( 'wp_head', 'wds_acme_display_customizer_header_scripts', 999 );
 ...
 */
function wds_acme_display_customizer_footer_scripts() {
	// Check for footer scripts.
	$scripts = get_theme_mod( 'wds_acme_footer_scripts' );
...
add_action( 'wp_footer', 'wds_acme_display_customizer_footer_scripts', 999 );
...
function wds_acme_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'touchstonesfse' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'touchstonesfse' ) : esc_html__( 'tagged with', 'touchstonesfse' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'touchstonesfse' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'touchstonesfse' ), $search_term );
		$card_description      = $card_long_description;
	}
...
add_action( 'wp_head', 'wds_acme_add_og_tags' );
 ...
 */
function wds_acme_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'wds_acme_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function wds_acme_disable_wpautop_for_gutenberg() {
...
add_filter( 'init', 'wds_acme_disable_wpautop_for_gutenberg', 9 );
