						...
						if ( has_action( 'woocommerce_account_downloads_column_' . $column_id ) ) {
							...
							switch ( $column_id ) {
								case 'download-product':
									...
									break;
								case 'download-file':
									echo '<a href="' . esc_url( $download['download_url'] ) . '" class="has-small-font-size wp-element-button woocommerce-MyAccount-downloads-file button alt">Download</a>';
									$type = pathinfo($download['file']['file'], PATHINFO_EXTENSION); 
									if($type == "mp3"){
										?>
										<a class="wp-element-button has-small-font-size" href="/audio-player/?prod=<?php echo $download['product_id']. "&order=".$download['order_id'] ?>">Listen</a>
										<?php
									}
									break;
